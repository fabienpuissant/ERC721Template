...
module.exports.mine = async (address, txHash) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (transaction !== undefined) {
      if (
        Web3.utils.toChecksumAddress(transaction.from) ===
...
module.exports.getNftByAddress = async (address) => {
  return new Promise(async (resolve, reject) => {
    let contract = getContract();
    const ammount = await contract.methods
      .balanceOf(address)
      .call()
      .catch((error) => {
        reject(error);
    ...
    for (let i = 0; i < ammount; i++) {
      let nftIndex = await contract.methods
        .tokenOfOwnerByIndex(address, i)
        .call()
        .catch((error) => {
          reject(error);
        });
      let nftHash = await contract.methods
        .tokenURI(nftIndex)
        .call()
        .catch((error) => {
          reject(error);
      ...
      let hashAttr = hashes.find((el) => {
        return el.hash === nftHash;
      });
      let nft = {};
...
const getContract = () => {
  ...
  let contract = new web3.eth.Contract(Contract.abi, contractAdress);
  return contract;
};
